...
<div id="cookies" style="line-height: normal; visibility:hidden;">
        ...
        <div id="content">
            <span class="bigtitle">TWOJE GRY</span>
            
            <div class="dottedline"></div>
            <?php

        $wynik = @$connection->query(sprintf("SELECT GryUzytkownikow.Id_gry As id_gry, GryUzytkownikow.Id_uzytkownika As id_user, Uzytkownicy.nazwa_uzytkownika As login, Gry.nazwa_gry As gra, Gatunek.nazwa_gatunek As  gatune, Producenci.Nazwa_producenta As Producent , Gry.data_wydania As data
            From GryUzytkownikow Join Gry On GryUzytkownikow.Id_gry = Gry.Id_gry
            Join Uzytkownicy on GryUzytkownikow.Id_uzytkownika = Uzytkownicy.Id_uzytkownika
            Join Producenci on Gry.id_producent = Producenci.ID_Producent
            Join Gatunek on Gry.id_gatunek = Gatunek.id_gatunek 
            WHERE Uzytkownicy.nazwa_uzytkownika = '%s' ", mysqli_real_escape_string($connection,$_COOKIE['login'])))
        or die('Błąd zapytania');
        ...
        while($r = $wynik->fetch_assoc()) 
        {
            $ID=$r['id_gry'];
            echo "<tr>";
            echo "<td><a href = 'gra.php?id=".$r['id_gry']."'>".$r['gra']."</a></td>";
            echo "<td>".$r['gatune']."</td>";
            echo "<td>".$r['Producent']."</td>";
            echo "<td>".$r['data']."</td>";
            echo "</tr>";
        }
        echo "</table>";
    ...
    <a href="index.php" align = "center"> Powrót do strony glownej</a>
</body>
